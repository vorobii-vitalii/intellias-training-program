...
public class WebSocketMessageReader implements MessageReader<WebSocketMessage> {
	...
	@Override
	public Pair<WebSocketMessage, Integer> read(BytesSource bytesSource, EventEmitter eventEmitter) throws ParseException {
		int N = bytesSource.size();
		if (N < METADATA_IN_BYTES) {
			return null;
		}
		var isFin = isFin(bytesSource);
		var isMasked = isMasked(bytesSource);
		var opCode = getOpCode(bytesSource);
		var payloadLength = getPayloadLength(bytesSource);
		...
		if (extraPayloadLengthBytes != 0) {
			var extendedSize = new BigInteger(
					1,
					extractBytes(bytesSource, METADATA_IN_BYTES, METADATA_IN_BYTES + extraPayloadLengthBytes));
		...
		var payload = extractBytes(bytesSource, payloadStart, payloadStart + payloadLength);
		if (isMasked) {
	...
	private int getPayloadLength(BytesSource bufferContext) {
		return bufferContext.get(1) & PAYLOAD_LENGTH_BITMASK;
	}

	private OpCode getOpCode(BytesSource bufferContext) throws ParseException {
		return OpCode.getByCode(bufferContext.get(0) & OP_CODE_BITMASK);
	}

	private boolean isFin(BytesSource bufferContext) {
		return (bufferContext.get(0) & IS_FIN_BITMASK) != 0;
	}

	private boolean isMasked(BytesSource bufferContext) {
		return (bufferContext.get(1) & IS_MASKED_BITMASK) != 0;
	}

	private byte[] extractBytes(BytesSource bufferContext, int start, int end) {
